...
class Frontend::HomeController < Frontend::BaseController
  def index
    ...
    @people_posts = Category.where(en_name: 'people').first.posts.order(published_at: :desc).published.limit(3)
    @place_posts = Category.where(en_name: 'places').first.posts.order(published_at: :desc).published.limit(3)
    @music_posts = Category.where(en_name: 'music').first.posts.order(published_at: :desc).published.limit(3)
    @speed_posts = Category.where(en_name: 'speed').first.posts.order(published_at: :desc).published.limit(3)
